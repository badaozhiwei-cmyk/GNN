...
class IL_set_v2(torch.utils.data.Dataset):
    """
    制冷剂-离子液体三图数据集（可解释性专用版）。

    __getitem__ 返回：
        Combine_Graph : PyG Data，三图拼接 + 全局节点
        condition     : FloatTensor [5]，归一化后的 [T, P, Ref_Charge, Ref_LogP, Ani_MW]
        label         : FloatTensor scalar，溶解度 x1
        num_bond      : int，添加全局节点之前的原始边数
    ...
    def __init__(self, data_npy_path: str, label_npy_path: str):
        """
        Args:
            data_npy_path  : processed_tri_data/data.npy 的路径
            label_npy_path : processed_tri_data/label.npy 的路径
        """
        super().__init__()

...
class IL_Explainer(torch.nn.Module):
    ...
    def __init__(self, model, epochs: int = 100, lr: float = 0.01,
                 ...
                 log: bool = False):
        super(IL_Explainer, self).__init__()
    ...
    # def explain_graph(self, x, edge_index, edge_weight):
    def explain_graph(self, graph, cond):
        ...
        with torch.no_grad():
            ...
            # out = self.model(x=x, edge_index=edge_index, batch=batch, edge_weight = edge_weight)
            # debug check dtype
            # print(graph.x.dtype,graph.edge_index.dtype,graph.edge_attr.dtype)
            out = self.model(graph,cond)
        ...
        for epoch in range(1, self.epochs + 1):
            optimizer.zero_grad()
            graph.x = (graph.x * self.node_feat_mask.view(1, -1).sigmoid()).to(torch.long)

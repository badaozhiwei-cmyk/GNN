...
gat_explain.py
===================================================
制冷剂-离子液体三图体系 单分子结构重要性热力图可视化 (V2) - GAT专版

主要功能：
  1. 通过命令行参数指定任意样本索引（--sample_idx）
  2. 对该样本加载 GAT 权重视角，运行 GNNExplainer 掩码优化
  3. 基于原子重要性分数绘制该分子的 2D 骨架热力图（重要原子呈红色渐变）
  4. 自动标注原子化学符号（如 C, F, O, N）

使用示例：
  python gat_explain.py --sample_idx 100 --model_path ../GNN_for_property_prediction/checkpoints_v2/best_gat_seed_42.pth
"""
...
def main(sample_idx: int, model_path: str, data_root: str, epochs: int):
    ...
    # ── 3. 运行 Explainer ──────────────────────
    print(f"[计算解释] 正在以 GAT 视角优化掩码，共 {epochs} 轮...")
    ...
    cbar.set_label('Atom Importance Score (GAT-Explainer)', rotation=270, labelpad=15, fontsize=10)
    ...
    plt.title(
        f"GAT Atom Importance Heatmap for Sample {sample_idx}\n"
    ...
    save_path = os.path.join(out_dir, f'gat_single_hotmap_idx_{sample_idx}.png')
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"🎉 [成功] GAT单分子结构重要性热力图已绘制并保存至:\n   {save_path}")
    print("\n💡 [分析小结] GAT 认为本体系中重要性最高的前 5 个原子为:")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='制冷剂体系单样本 GAT-Explainer 结构热力图分析 V2')
    parser.add_argument('--sample_idx', type=int, default=100, help='需要分析的样本索引，范围 0~4443')
    parser.add_argument('--model_path', type=str, default=os.path.join(ROOT, 'GNN_for_property_prediction', 'checkpoints_v2', 'best_gat_seed_42.pth'), help='GAT 的权重路径')

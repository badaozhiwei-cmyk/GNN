"""
compute_full_pair_interaction_xtb.py — Pairwise Association Energy Pipeline (v2.0 Rigorous)
==========================================================================================
...
Rigorous Methodological Protocol:
  1. Multi-orientation initial structure sampling (4 spatial orientations per pair)
  2. Geometry optimization via GFN2-xTB with tight convergence criteria
  3. Lowest-energy converged structure selection among the four sampled initial orientations
     (Explicitly NOT claiming global energy minimum)
  4. Association Energy Formulation:
     Delta_E_assoc = E_complex - (E_ion_opt + E_ref_opt)
     (Relative to isolated optimized monomers; includes intermolecular binding and geometry relaxation)
  5. Full Intermediate Audit Retention:
     Preserves E_ori1..4, converged_ori1..4, d_min_ori1..4, N_Converged_Orientations, and Best_Orientation.
==========================================================================================
"""
...
def write_xyz(atom_symbols, coords, filepath):
    with open(filepath, 'w') as f:
        f.write(f"{len(atom_symbols)}\nGenerated by Full Pairwise Interaction Pipeline\n")
...
def adjust_min_distance(ion_xyz, ref_xyz, target_dist=3.2, tol=1e-3, max_iter=10):
    """
    调整两个分子片段之间的最近原子间距至 target_dist (默认 3.2 Å)。
...
def main():
    print("="*75)
    print("  Full Dataset GFN2-xTB Pairwise Association Energy Pipeline (v2.0 Rigorous)")
    print("="*75)
    ...
    for idx, (pair_type, ion_name, ion_smi, ion_q, ref_name, ref_smi, ref_q, dimer_q) in enumerate(pair_tasks, 1):
        ...
        if converged_candidates:
            ...
            print(f"   🏆 4个构向中最低能量收敛解 (Ori {best_ori}): ΔE_assoc = {delta_e_assoc if delta_e_assoc else 0.0:.2f} kcal/mol | d_min = {best_d_min:.2f} Å (收敛 {len(converged_candidates)}/4)")
            ...
            res_row = {
                ...
                'Delta_E_int_kcal_mol': delta_e_assoc,  # 兼容历史脚本调用的同义别名
    ...
    print("\n" + "="*75)
    print(f"🎉 全部 218 组配对缔合能计算完成！结果保存在: {OUTPUT_CSV}")
    print("="*75)

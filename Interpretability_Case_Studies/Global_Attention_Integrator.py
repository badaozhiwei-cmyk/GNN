...
def main():
    ...
    # 步骤二：读取数据
    print("\n[进度 3/5] 正在读取全宇宙 4444 条原始数据 (ZLJ_DATA.xlsx)...")
    ...
    total_len = len(df)
    print(f"   => 成功读取全宇宙记录数: {total_len} 条")
    ...
    sub_len = len(sub_df)
    print(f"   => 🎯 精准锁定子宇宙！该特定大体系下共计 {sub_len} 条数据 (占比 {sub_len/total_len*100:.1f}%)")
    ...
    # 步骤四：开始暴力积分
    print("\n[进度 5/5] 🚀 开始批量 GAT 推断，收集制冷剂吸收的『靶向注意力碎片』...")
    
    # 记录制冷剂内部，各个元素的总得分
    element_score_bank = defaultdict(float)
    valid_count = 0
    
    for idx, row in tqdm(sub_df.iterrows(), total=sub_len, desc="GAT 积分中"):
        cat = row['IL cation']
        ani = row['IL anion']
        ref = row['Refrigerant']
        T = row['T (K)']

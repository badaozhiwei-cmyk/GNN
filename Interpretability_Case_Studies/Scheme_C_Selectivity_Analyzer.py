...
def main():
    ...
    if not os.path.exists(model_path):
        print(f"找不到权重文件 {model_path}，请确保您在 Kaggle 上正确配置了路径。")
        return
    ...
    # 分离出阳离子 (mol=0) 和 阴离子 (mol=1) 的总注意力
    ...
    print(f"\n当吸收 {r1_name} 时:")
    print(f"  ➜ [BMIM]+ 关注度: {cat_ratio1:.1f}%")
    print(f"  ➜ [Tf2N]- 关注度: {ani_ratio1:.1f}%")

    print(f"\n当吸收 {r2_name} 时:")
    print(f"  ➜ [BMIM]+ 关注度: {cat_ratio2:.1f}%")
    print(f"  ➜ [Tf2N]- 关注度: {ani_ratio2:.1f}%")

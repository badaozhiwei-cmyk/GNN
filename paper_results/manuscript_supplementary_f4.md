...
## 1. Quantitative Efficiency Summary
       ...
       Architecture                       MLP Readout Head Parameters                                      1,062,913           count
         ...
         Complexity                        Batched Compute (Batch=32)                                         36.275  GFLOPs / batch
            Latency                         Single-Sample CPU Latency                                   13.28 ± 1.85              ms
            Latency                         P95 Single-Sample Latency                                          15.70              ms
            Latency                    Batched CPU Latency (Batch=32)                                         349.46              ms
         Throughput                            Batched CPU Throughput                                           91.6   samples / sec
             Memory                   Process RSS at Measurement Time                         817.8 MB (Process RSS)          memory
...
3. **Empirical Latency & Throughput**: Benchmarked via CPU inference under the reported runtime environment, single-sample evaluation incurs **13.28 ± 1.85 ms** (P95: 15.70 ms). Under batched execution (batch size = 32), throughput reaches **91.6 samples / second** (10.92 ms per sample). This enables screening large thermodynamic candidate spaces (e.g., >100,000 ternary combinations in under 25 minutes on standard workstations).

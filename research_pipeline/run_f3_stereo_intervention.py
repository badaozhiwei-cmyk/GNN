...
def main():
    ...
    with open(out_report, "w", encoding="utf-8") as f:
        ...
        f.write("---\n\n")
        f.write("## 1. Executive Scientific Verdict: Empirical Evidence Consistent with Scalar Bypass\n\n")
        ...
        f.write("## 2. Mechanistic Insight: Empirical Graph-to-Head Insensitivity Under the Tested Checkpoint\n\n")
        f.write("> **Core Epistemological Finding**: Under the tested checkpoint (Seed 42), **possessing stereochemical feature capacity does not result in downstream readout utilization**.\n\n")
        f.write("1. **Contextualizing with Step 25 Representation Bifurcation**: In Step 25, substructure attribution revealed a seed-dependent representation bifurcation across 43 evaluation probes × 5 seeds, where the majority of evaluations exhibited readout collapse or head insensitivity, with the final MLP relying primarily on scalar condition shortcuts.\n")
        f.write("2. **Empirical Evidence of Graph-to-Head Insensitivity**: The F3 intervention provides empirical evidence consistent with this bypass behavior: for this tested checkpoint, enriching topological edge representations ($G_E \\neq G_Z$) does not propagate to zero-shot output differences, indicating that graph representation channels remain unutilized by the readout head.\n")
        f.write("3. **Architectural Implication (V7 Future Work)**: This observation indicates that resolving 2D topological isomer degeneracy likely requires pairing explicit stereochemical featurization with inductive architectures that actively discourage scalar shortcut learning (e.g., condition dropout or component-level interaction pooling).\n")
        f.write("4. **Scope Restriction**: This finding characterizes the empirical behavior of the specific evaluated checkpoint (Seed 42) and cohort; it serves as empirical evidence of checkpoint-level graph-to-head insensitivity rather than an assertable universal mechanism across all model instances or architectures.\n\n")
        f.write("---\n\n")

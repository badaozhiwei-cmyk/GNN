...
Args = {
    ...
    # GCN model hyperparameter (3-layer version)
    # n_features=7: atomic_number, hybridization, aromatic, degree, charge, electronegativity, cov_radius
    'dropout_rate': 0.4,
    'n_features': 7,
    'warmup': 40,
...
class Runner(object):
    """
    Runner for GCN training, validation and testing.
    Architecture: 3-layer GCNConv + MLP head (mirrors GIN_Runner structure)
    """
    def __init__(self, args, seed=42):
        ...
        self._optimizer = torch.optim.Adam(
            self._model.parameters(), lr=args['lr'], weight_decay=args['weight_decay']
        )
    ...
    def train(self, train_loader, dev_loader, args):
        ...
        for epoch in range(1, args['epoch'] + 1):
            ...
            batch_bar = tqdm(
                ...
                position=0, leave=False, desc=f'Epoch {epoch} Train'
            )
            ...
            for batch_idx, (graph, cond, label) in enumerate(train_loader):
                ...
                batch_bar.set_postfix(
                    loss="{:.04f}".format(train_loss / (batch_idx + 1)),
            ...
            if avg_val_loss < best_v_loss:
                ...
                self._save_para('best_gcn')

            scheduler.step(avg_val_loss)

            print(f"Epoch {epoch}/{args['epoch']}: Train loss {avg_train_loss:.04f}, "
                  f"Val loss {avg_val_loss:.04f}, LR {optimizer.param_groups[0]['lr']:.06f}")
    ...
    def test(self, test_loader):
        ...
        checkpoint_path = f"checkpoints/best_gcn_seed_{self.seed}.pth"
...
def plot_results(true_y, pred_y, title, filename):
    ...
    plt.scatter(true_y, pred_y, alpha=0.5, color='darkorange', label='Predictions')
...
if __name__ == '__main__':
    ...
    # ============================================================
    # 固定数据切分 seed，保证 GCN 与 GIN 在完全相同的测试集上对比
    # ============================================================
    SPLIT_SEED = 42
    ...
    for seed in seeds:
        ...
        print(f"  Single GCN model → MAE: {mae:.4f}, R²: {r2:.4f}")

        plot_results(test_true, test_pred, f"GCN Seed {seed} Prediction", f"gcn_pred_seed_{seed}")
    ...
    print(f"\n{'='*20} Individual GCN Model Summary {'='*20}")
    ...
    print(f"\n{'='*20} GCN Ensemble Result {'='*20}")
    print(f"Ensemble MAE: {ensemble_mae:.4f}")
    print(f"Ensemble R²:  {ensemble_r2:.4f}  ← GCN 集成最终 R²")

    df_results['ensemble_mae'] = ensemble_mae
    df_results['ensemble_r2']  = ensemble_r2
    df_results.to_csv('gcn_ensemble_results_summary.csv', index=False)
    print("Summary saved to gcn_ensemble_results_summary.csv")

    plot_results(test_true, ensemble_pred,
                 f"GCN Ensemble Prediction (R²={ensemble_r2:.4f})",
                 "gcn_ensemble_prediction_final")